...
class FoodClassifier:
    ...
    def _load_class_names(self) -> None:
        """Load the ordered class list saved by the training script."""
        # The training script writes class_names.json next to the model checkpoints.
        # This order MUST match the model's output indices, so prefer it over
        # class_mapping.json (which is only for nutrition lookups and may be partial).
        names_path = settings.MODEL_DIR / "class_names.json"
        ...
        if mapping_path.exists():
            ...
            logger.warning(
                ...
                msg="class_names.json missing; falling back to class_mapping.json keys (order may not match model).",
            )
        else:
            logger.warning(
                "class_mapping_missing",
                path=str(mapping_path),
                msg="No class list found. Predictions will be 'unknown'.",
            )
